...
# ====================== Sección #2: Cálculos Eléctricos Básicos ======================
  ...
  - Subsección #2.2: Cálculo de cargas y potencias
    - Función #2.2.1: calcular_kva_usuario(area_lote) - Estima kVA demandado por usuario según área
    - Función #2.2.2: factor_coincidencia(usuarios) - Factor de coincidencia según número de usuarios
    - Función #2.2.3: calcular_potencias(df, kva_usuario, factor_coinc, fp) - Calcula potencias activas, reactivas y complejas
    - Función #2.2.4: calcular_impedancia_admitancia(df, V_nom) - Obtiene impedancia y admitancia de cargas

  - Subsección #2.3: Preparación de matriz nodal
    - Función #2.3.1: obtener_nodos_e_indices(df) - Lista ordenada de nodos y diccionario índice por nodo
    - Función #2.3.2: construir_matriz_admitancia(df, nodos, indice_nodos) - Construye matriz nodal Y
    - Función #2.3.3: extraer_submatrices(Y, nodos) - Extrae submatrices Yrr, vector Y_r0, y nodo slack



# ====================== Sección #3: Cálculo de Voltajes y Regulación ======================
  - Subsección #3.1: Cálculo y reporte de voltajes nodales
    - Función #3.1.1: calcular_voltajes_nodales(Yrr, Y_r0, slack_index, nodos, V0) - Calcula voltajes nodales y retorna DataFrame
    - Función #3.1.2: calcular_regulacion_voltaje(V, nodos, nodo_slack) - Calcula regulación de voltaje para cada nodo



# ====================== Sección #4: Cálculo de Corrientes, Pérdidas y Proyección ======================
  - Subsección #4.1: Corrientes y pérdidas en vanos
    - Función #4.1.1: calcular_corrientes(df, V) - Calcula corrientes por tramo y agrega al DataFrame
    - Función #4.1.2: calcular_perdidas_y_proyeccion(df) - Calcula pérdidas eléctricas y proyecta pérdidas futuras
    
  - Subsección #4.2: Proyección de demanda y carga
    - Función #4.2.1: proyectar_demanda(potencia_total_kva, df_parametros, df_proyeccion, crecimiento, años) - Proyecta demanda futura y porcentaje de carga
...
# ====================== Sección #9: Generación de Informe PDF ======================
- Subsección #9.1: Generación y armado del documento PDF
  - Función #9.1.1: crear_pdf(nombre_archivo, secciones, nodos_inicio, nodos_final, usuarios, distancias, capacidad_transformador) - Crea PDF con texto e imagen de red
  - Función #9.1.2: cargar_y_preparar_datos(archivo) - Carga datos, calcula parámetros eléctricos y prepara DataFrame
  - Función #9.1.3: calcular_flujo_carga_y_perdidas(df_conexiones, df_parametros, area_lote) - Flujo de carga, voltajes, pérdidas y proyecciones
  - Función #9.1.4: calcular_regulacion_y_proyeccion(potencia_total_kva, df_parametros, Yrr, Y_r0, slack_index, nodos, nodo_slack) - Proyección demanda, voltajes y regulación
  - Función #9.1.5: preparar_proyeccion_y_regulacion(...) - Alias para preparar DataFrames de proyección y regulación
  - Función #9.1.6: generar_informe_pdf(df_info, df_parametros, factor_coinc, df_conexiones, df_voltajes, df_regulacion, capacidad_transformador, df_proyeccion, proyeccion_perdidas, perdida_total, nodos_inicio, nodos_final, usuarios, distancias) - Orquesta creación del informe PDF con secciones y gráficos
...
- Subsección #11.1: Creación y armado del informe en PDF con reportlab
  - Función #11.1.1: crear_pdf(nombre_archivo, secciones, nodos_inicio, nodos_final, usuarios, distancias, capacidad_transformador) - Genera PDF con todas las secciones y gráficos incluidos
  - Función #11.1.2: cargar_y_preparar_datos(archivo) - Carga datos, calcula parámetros y prepara DataFrames y listas
  - Función #11.1.3: calcular_flujo_carga_y_perdidas(df_conexiones, df_parametros, area_lote) - Ejecuta flujo de carga, voltajes, corrientes, pérdidas y proyecciones
  - Función #11.1.4: calcular_regulacion_y_proyeccion(potencia_total_kva, df_parametros, Yrr, Y_r0, slack_index, nodos, nodo_slack) - Calcula proyección de demanda y regulación de voltajes
  - Función #11.1.5: preparar_proyeccion_y_regulacion(...) - Prepara DataFrames de proyección y regulación para informe
  - Función #11.1.6: generar_informe_pdf(df_info, df_parametros, factor_coinc, df_conexiones, df_voltajes, df_regulacion, capacidad_transformador, df_proyeccion, proyeccion_perdidas, perdida_total, nodos_inicio, nodos_final, usuarios, distancias) - Orquesta la creación final del PDF con datos, gráficos y secciones
...
#Sección 1.2 Definición de Variables y funciones. 
#Se definen todas las variables requeridas. 


#Sección 1.2 Definición de funciones. 
#Se definen todas las variables requeridas. 
...
def calcular_kva_usuario(area_lote):
    # Ejemplo simple, puedes ajustar la fórmula real aquí
    return 0.5 + 0.01 * area_lote
...
def calcular_potencia_carga(df, area_lote, fp=0.9, V_nom=240):
    total_usuarios = df['usuarios'].sum()
    kva_usuario = calcular_kva_usuario(area_lote)
...
def crear_seccion_perdidas_y_corrientes(df_resultado, perdida_total, proyeccion):
    elementos = []
    elementos.append(crear_subtitulo("Corrientes y Pérdidas por Vano"))
    
    # Preparar tabla simplificada para PDF (con columnas necesarias)
    df_tabla = df_resultado[['nodo_inicial', 'nodo_final', 'I_vano', 'P_perdida']].copy()
    df_tabla['|I| (A)'] = df_tabla['I_vano'].apply(lambda x: abs(x))
    df_tabla = df_tabla.rename(columns={
        'nodo_inicial': 'Nodo Inicial',
        'nodo_final': 'Nodo Final',
        'P_perdida': 'Pérdida (W)'
    })
    # Eliminar la columna compleja para mostrar solo magnitud
    df_tabla = df_tabla.drop(columns=['I_vano'])
    
    tabla = crear_tabla(df_tabla, encabezados=df_tabla.columns.tolist())
    elementos.append(tabla)
    elementos.append(Spacer(1, 12))
    
    # Añadir resumen de pérdidas totales
    resumen = Paragraph(
        f"Pérdida total anual estimada: {perdida_total:.2f} kWh", 
        ParagraphStyle(name='Normal', fontSize=12)
    )
    elementos.append(resumen)
    
    # Añadir gráfico o lista con proyección (opcional)
...
# ====================== Sección #9: Proyección de Demanda y Análisis de Cargabilidad ======================
...
def preparar_info_proyecto(df_info, df_parametros, factor_coinc):
    ...
    fila_factor = pd.DataFrame([{'Parámetro': 'Factor de Coincidencia', 'Valor': f"{factor_coinc:.2f}"}])
    df_parametros_copy = pd.concat([df_parametros_copy, fila_factor], ignore_index=True)
...
def seccion_info_proyecto(df_info_copy, df_parametros_copy):
    elementos = []
...
def crear_tabla_usuarios_conectados(df_conexiones):
    ...
    df_tabla = df_clean[['nodo_final', 'distancia', 'usuarios']].copy()
    df_tabla.columns = ["Nodo", "Distancia (m)", "Usuarios"]
    return crear_tabla(df_tabla, encabezados=df_tabla.columns.tolist())
...
def crear_tabla_voltajes(df_voltajes, df_regulacion):
    ...
    tabla = crear_tabla(df_tabla, alineacion='LEFT')
    elementos.append(tabla)
    elementos.append(Spacer(1, 12))
...
def generar_informe_pdf(df_info, df_parametros, factor_coinc, df_conexiones, df_voltajes, df_regulacion, capacidad_transformador, df_proyeccion, proyeccion_perdidas, perdida_total, nodos_inicio, nodos_final, usuarios, distancias):
    df_info_copy, df_parametros_copy = preparar_info_proyecto(df_info, df_parametros, factor_coinc)

    secciones = [
        seccion_info_proyecto(df_info_copy, df_parametros_copy),
...
def main():
    ...
    (df_conexiones, df_parametros, df_info, tipo_conductor,area_lote,capacidad_transformador, usuarios,distancias, nodos_inicio, nodos_final) = cargar_y_preparar_datos(archivo)   
    ...
    generar_informe_pdf(df_info, df_parametros, factor_coinc, df_conexiones, df_voltajes, df_regulacion, capacidad_transformador, df_proyeccion, proyeccion_perdidas, perdida_total, nodos_inicio, nodos_final, usuarios, distancias)
